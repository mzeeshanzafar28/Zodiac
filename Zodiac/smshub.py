...
class smshubverification:
    ...
    def ordernumber(self):
        ...
        def smshub_internal_error_handle(self):
            if "BAD_KEY" in response.text:
                pystyle.Write.Print(f"\t-> *** Could not order the Number because API Key is Invalid ***\n", pystyle.Colors.orange, interval=0)
                sys.exit(1)
            
            elif "BAD_SERVICE" in response.text:
                pystyle.Write.Print(f"\t-> *** Could not order the Number because Service Name is Incorrect ***\n", pystyle.Colors.orange, interval=0)
                sys.exit(1)
    ...
    def getcode(self):
        ...
        while response.text == "STATUS_WAIT_CODE": 
            ...
            print(f"\r\t-> Discord SMS seems stuck. Waiting... {waitcount}/400", end="", flush=True)
            ...
            if waitcount % 5 == 0: # run every x time to request a new sms from discord
                ...
                if "message" in discordresponse:
                    if "message" == "The resource is being rate limited.":
                        ...
                        pystyle.Write.Print(f"\-> Ratelimited for requesting a new SMS ({str(ratelimit_duration)} Seconds, trying to get the SMS without requesting a new one...\n", pystyle.Colors.red, interval=0)
                        ratelimited = True
